    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      switch (event.key) {
        case "F8":
          ...
          if (paused) window.dispatch({ event: "resume" });
          else window.dispatch({ event: "pause" });
          break;
        case "F10":
          event.preventDefault();
          if (paused) window.dispatch({ event: "step" });
          break;
      }
    };
      ...
      <Toolbar>
        <ToolbarButton
          ...
          onClick={() => {
            window.dispatch({
              ...
              params: {
                mode:
                  mode === "none" || mode === "standby" || mode === "inspecting"
                    ? "recording"
                    : "standby",
              },
            });
        ...
        <ToolbarButton
          ...
          onClick={() => {
            ...
            window
              .dispatch({ event: "setMode", params: { mode: newMode } })
              .catch(() => {});
          }}
        ></ToolbarButton>
        <ToolbarButton
        ...
        <ToolbarButton
          ...
          onClick={() => {
            window.dispatch({
              event: "setMode",
              params: {
                mode: mode === "assertingText" ? "recording" : "assertingText",
              },
            });
          }}
        ></ToolbarButton>
        <ToolbarButton
          ...
          onClick={() => {
            window.dispatch({
              ...
              params: {
                mode:
                  mode === "assertingValue" ? "recording" : "assertingValue",
              },
            });
          }}
        ></ToolbarButton>
        ...
        <select
          ...
          onChange={(event) => {
            ...
            window.dispatch({
              event: "fileChanged",
              params: { file: event.target.selectedOptions[0].value },
            });
          }}
        >
    ...
    return [...groups].filter(Boolean).map((group) => (
      <optgroup label={group} key={group}>
        {sources
          ...
          .map((source) => renderOption(source))}
      </optgroup>
    ));
  }
